...
public class Stress {
    ...
    public static void main(String[] args) throws Exception {
        ...
        parser.accepts("n", "Number of requests to perform (default = unlimited)").withRequiredArg().ofType(Integer.class).defaultsTo(-1);
        parser.accepts("t", "Level of concurrency to use").withRequiredArg().ofType(Integer.class).defaultsTo(50);
        parser.accepts("async", "Make asynchronous requests instead of blocking");
        ...
        int requests = (Integer)options.valueOf("n");
        ...
        try {
            ...
            for (int i = 0; i < concurrency; i++) {
                ...
                QueryGenerator generator = generators.get(action).create(i, iterations, options, session);
